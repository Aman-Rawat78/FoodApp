...
import {  PasswordResetState, userPasswordResetSchema } from "@/shcema/userSchema";
...
const ResetPassword = () => {
  const navigate = useNavigate();
    ...
    const [errors, setErrors] = useState<Partial<PasswordResetState>>({});
    const params = useParams<{ token: string }>(); // Extract the token from the URL parameters
    
   const handleResetPassword = async (e:FormEvent)=>{
        e.preventDefault();
   
        const result = userPasswordResetSchema.safeParse({password:newPassword})
        if(!result.success){
            const fieldErrors = result.error.formErrors.fieldErrors;
            setErrors(fieldErrors as Partial<PasswordResetState>);
            return;
        }

        try {
          await resetPassword(params.token!, newPassword); // Pass the token and new password
          navigate("/login");
        } catch (error) {
    ...
    <div className="flex items-center justify-center min-h-screen w-full">
      <form onSubmit={handleResetPassword} className="flex flex-col gap-5 md:p-8 w-full max-w-md rounded-lg mx-4">
        ...
        <div className="relative w-full">
            ...
            {errors && (
              <span className="text-xs text-red-500 text-left block ml-4">{errors.password}</span>
            )}
        </div>
        {
            loading ? (
